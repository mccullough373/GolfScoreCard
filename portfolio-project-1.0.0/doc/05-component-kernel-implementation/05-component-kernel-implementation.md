...
For my representation, I plan to use a sequence to store the score for each hole
as well as an int to easily add to teh sequence. I thought that this
representation would be the best overall for my design and would also be really
easy to implement as I sort of already have experience with using a sequence in
the proof of concept assignment. For the convention I was going to allow for the
sequence to be any size, but restrict some of the methods such as `frontNine()`
and `backNine()` to still only use the first 18 elements.
