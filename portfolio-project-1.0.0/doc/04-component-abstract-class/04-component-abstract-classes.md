...
I added a kernel method for `currentStrokes()` which just returns the current
strokes taken on the current hole. I just felt like this method could be useful
for overall functionality. Also. I added two constant variables in the Kernel
class, as they are used for the `frontNine()` and `backNine()` methods. Lastly,
the secondary method `totalScore()` had its required clause removed as it no
longer requires there to be a hole played for it to work.
